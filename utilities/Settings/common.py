...
def getDriver():
    ...
    Write.Print("\nChecking Downloaded Drivers!", Colors.green_to_white, interval=0.015)
    sleep(0.5)
    for driver in drivers:
        if os.path.exists(os.getcwd() + os.sep + driver):
            Write.Print("\nChromeDriver is Already Installed...", Colors.green_to_white, interval=0.015)
            sleep(0.5)
            return driver
    else:
        Write.Print("\nInstalling Drivers!\n\n", Colors.green_to_white, interval=0.015)
        if os.path.exists(os.getenv('localappdata') + '\\Google'):
            Chrome_Installer()
            Write.Print("\nChromeDriver.exe Has Been Installed Successfully!", Colors.green_to_white, interval=0.015)
...
def loadbar(iteration, total, prefix='', suffix='', decimals=1, length=100, fill='#'):
    ...
    Write.Print(f"\r                                       {prefix} |{bar}| {percent}% {suffix}", Colors.green_to_white, interval=0.000)
    if iteration == total:
        print()
...
def proxy_scrape(): 
    startTime = time.time()
    temp = getTempDir()+"\\shrek_proxies"                               
#    Write.Print("\n\n\n\n\n\n\n\n\n\n                                          Collecting Proxies | Please Wait... \n", Colors.green_to_white, interval=0.010)


    execution_time = (time.time() - startTime)
#    Write.Print(f"                                                        Opening Menu", Colors.green_to_white, interval=0.000)

def setTitle(_str):
    system = os.name
    if system == 'nt':
        ctypes.windll.kernel32.SetConsoleTitleW(f"{_str}Shrek Multi Tools | Made by Shrek™| Tokens: [{counttokens}]")
    elif system == 'posix':
...
System.Size(120, 30)
System.Clear()
Anime.Fade(Center.Center(shrek), Colors.green_to_white, Colorate.Vertical, interval=0.030, enter=True)
